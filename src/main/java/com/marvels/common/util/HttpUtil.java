...
public class HttpUtil {
    ...
    /**获取secret参数**/
    private static  String secret = PropertiesLoadUtil.getPropertiesValue("qj.jiufu.httpHand.secret", "forms-openapi.properties");
    /**
     * 设置hander头
     */
    /**商户编码(金融云平台给各商户分配的编码)**/
    private static  String code = PropertiesLoadUtil.getPropertiesValue("qj.jiufu.httpHand.code", "forms-openapi.properties");
    /**资金机构编号(本次业务请求对应的资金机构编号)**/
    private static String orgCode = PropertiesLoadUtil.getPropertiesValue("qj.jiufu.httpHand.orgCode", "forms-openapi.properties");;
     ...
     * @return 所代表远程资源的响应结果
     */
    public static String doPostByJiuFu(String interFaceCode,String httpUrl, String param) {
        //获取当前时间戳
        Long reqTime = System.currentTimeMillis();
        //请求时间戳(请求时间对应的毫秒数)
        String time = reqTime.toString();
        //接口编号(每个接口的编号)
        String intfCode = interFaceCode;
        /**请求签名(md5(code+time+secret)，其中，secret为金融云平台分配给商户的密钥)*/
        String sign = EncryptUtil.MD5Encode(code+time+secret);
        ...
        try {
            ...
            connection.setReadTimeout(60000);
            //设置hander信息
            connection.setRequestProperty("intfCode",intfCode);
            connection.setRequestProperty("code",code);
            connection.setRequestProperty("orgCode",orgCode);
